      ...
      <div class="modal-body">
        <form action="javascript:void(0)" id="adminform" name="adminform" class="form-horizontal" method="POST"
          ...
          <div class="form-group">
            ...
            <div class="col-sm-12">
              <input type="file" class="form-control" id="adminpic" name="adminpic" placeholder="admin pic" required="">
            </div>
          </div>
      ...
      <div class="modal-body">
        <form action="javascript:void(0)" id="editadminform" name="editadminform" class="form-horizontal" method="POST"
          ...
          <div class="form-group">
            <label class="col-sm-2 control-label">Pic</label>
            <div class="col-sm-12">
              <input type="file" class="form-control" id="adminpic1" name="adminpic" placeholder="admin pic">
            </div>
          </div>
          <div class="col-sm-offset-2 col-sm-10">
...
<script type="text/javascript">
  ...
  function editFunc(id) {
    ...
    $.ajax({
      type: "GET",
      url: "{{ url('superadmin/adminedit/{id}')}}",
      data: { id: id },
      dataType: 'json',
      success: function (res) {
        $('#editadmin').html("Edit Admin");
        $('#admin-edit-modal').modal('show');
        $('#id1').val(res.id);
        $('#name1').val(res.name);
        $('#address1').val(res.admin.address);
        $('#email1').val(res.email);
        $('#cnic1').val(res.admin.cnic);
        $('#cnicexp1').val(res.admin.cnicexpiry);
        $('#aimage1').css('display', 'block');
        $('#location1').val(res.admin.location);
        $('#aimage1').attr('src', '/storage/'+res.admin.pic);
        $('#status1').val(res.admin.status).change();
  ...
  function deleteFunc(id) {
    if (confirm("Delete Record?") == true) {
      ...
      $.ajax({
        type: 'DELETE',
        url: "{{ url('superadmin/admindelete') }}",
        data: { id: id },
        dataType: 'json',
        success: function (res) {
          console.log(res);
  ...
  $('#editadminform').submit(function (e) {
    ...
    $.ajax({
      type: 'POST',
      url: "{{ url('superadmin/adminupdate')}}",
      data: formData,
      cache: false,
      contentType: false,
      processData: false,
